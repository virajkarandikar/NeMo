...
class EncDecRNNTBPEMultiModel(EncDecRNNTModel, ASRAGGBPEMixin):
    ...
    def change_tokenizer(self, new_tokenizer_cfg: DictConfig,  decoding_cfg: Optional[DictConfig] = None):
        ...
        """
        Changes tokenizer used during RNNT decoding process. Use this method when fine-tuning on from pre-trained model.
        This method changes only decoder and leaves encoder and pre-processing modules unchanged. For example, you would
        use it if you want to use pretrained encoder when fine-tuning on data in another language, or when you'd need
        model to learn capitalization, punctuation and/or special characters.

        Args:
            new_tokenizer_cfg: A config for the new tokenizer
...
#         if new_tokenizer_type.lower() not in ('bpe', 'wpe', 'agg'):
#            raise ValueError(f'New tokenizer type must be either `bpe` or `wpe` or 'agg'')

#         tokenizer_cfg = OmegaConf.create({'dir': new_tokenizer_dir, 'type': new_tokenizer_type})

        # Setup the tokenizer
        self._setup_tokenizer(new_tokenizer_cfg)

        # replace my tokenizer config with this tokenizer
        self.cfg.tokenizer = new_tokenizer_cfg
        ...
        del self.joint
        self.joint = EncDecRNNTBPEMultiModel.from_config_dict(new_joint_config)

        decoder_config = self.decoder.to_config_dict()
        new_decoder_config = copy.deepcopy(decoder_config)
        new_decoder_config.vocab_size = len(vocabulary)
        del self.decoder
        self.decoder = EncDecRNNTBPEMultiModel.from_config_dict(new_decoder_config)
